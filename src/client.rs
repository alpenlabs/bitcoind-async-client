...
impl Client {
    ...
    async fn call<T: de::DeserializeOwned + fmt::Debug>(
        ...
        trace!(method = %method, "Calling bitcoin client with params: {:?}", params);

        self.client.request(method, params).await.map_err(|e| {
            error!(error = %e, "Error calling bitcoin client");
...
impl Wallet for Client {
    ...
    async fn create_raw_transaction(
        ...
        consensus::encode::deserialize_hex(&raw_tx).map_err(|e| {
